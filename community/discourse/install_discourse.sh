...
sed -i -e 's/^pidfile \/var\/run\/redis_6379.pid/pidfile \/home\/'"$SHELL_USER"'\/apps\/'"$APP_NAME"'\/tmp\/redis.pid/' redis.conf
sed -i -e 's/^logfile ""/logfile \/home\/'"$SHELL_USER"'\/logs\/'"$APP_NAME"'\/redis.log/' redis.conf
sed -i -e 's/daemonize no/daemonize yes/' redis.conf

# start redis
/home/$SHELL_USER/apps/$APP_NAME/redis-stable/src/redis-server /home/$SHELL_USER/apps/$APP_NAME/redis-stable/redis.conf 

# install discourse
cd /home/$SHELL_USER/apps/$APP_NAME
...
sed -ie 's/discourse\/discourse/'"$SHELL_USER"'\/apps\/'"$APP_NAME"'\/discourse/' config/puma.rb
