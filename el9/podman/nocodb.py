...
IMG = 'docker.io/nocodb/nocodb:latest'
...
def ensure_pg_same_server(opal_token, appinfo, prefix):
    ...
    return f"pg://{uname}:{upass}@{host}:5432/{dname}?sslmode=require"

def main():
    p = argparse.ArgumentParser(description='Installs NocoDB (Podman) on Opalstack')
    ...
    nc_db = ensure_pg_same_server(api.token, app, prefix='nc')

    env = textwrap.dedent(f"""\
    # Clear NC_DB to use SQLite instead
    NC_DB="{nc_db}"
    NC_ADMIN_EMAIL="admin@example.com"
    NC_ADMIN_PASSWORD="{secrets.token_urlsafe(12)}"
    """)
    ...
    podman run -d --name "$APP" \\
      ...
      -v "$APPDIR/data:/usr/app/data" \\
      --env-file "$APPDIR/.env" \\
      --label io.containers.autoupdate=registry \\
      "$IMG"
    echo "Started NocoDB for {app['name']} on 127.0.0.1:{port}"
    """)
    ...
    create_file(f'{appdir}/README.txt', f"NocoDB on port {port}. Data in {appdir}/data. Using managed PG on same server.\n", perms=0o600)
