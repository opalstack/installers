#!/usr/bin/env python3
# NocoDB — rootless Podman, single container (SQLite by default)
...
API_BASE='/api/v1'; IMG='docker.io/nocodb/nocodb:latest'
def sh(c,q=False): 
...
env=textwrap.dedent("""\
# Optional: set NC_DB config to use Postgres instead of SQLite
# NC_DB="pg://user:pass@host:5432/dbname?sslmode=require"
...
""")
readme=f"NocoDB on port {port}. Data in {appdir}/data. For Postgres, set NC_DB in .env."
...
print("nocodb installed",name,port,osuser)
